...
class Detector:
    ...
    def rescale_DPD_run_length_log_distribution(self, t):
        ...
        if self.jlp_scale is None:
            ...
            #            log_scale_new = max(1.0, utils.logsumexp([
            ...
            #                     utils.logsumexp([model.joint_log_probabilities
            #                        for model in self.model_universe]))
            #            log_scale_new = utils.logsumexp(
            #                    [model.joint_log_probabilities
            #                        for model in self.model_universe])
            log_scale_new = min_  # 0.5 * max(max_-min_, 2) #min(log_scale_old,
            #                    (np.max([model.joint_log_probabilities
            #                        for model in self.model_universe]) - 1))
            #            log_scale_new = max(1.0, utils.logsumexp([model.joint_log_probabilities
            #                        for model in self.model_universe]))
    ...
    #        log_scale_new = utils.logsumexp([model.joint_log_probabilities
    #                        for model in self.model_universe])
    ...
    def update_alpha_param(self, y, t, update=True):
        ...
        if self.alpha_param_learning == "together":
            ...
            total_evidence_p_eps = utils.logsumexp(list_model_log_evidences_p_eps)
            total_evidence_m_eps = utils.logsumexp(list_model_log_evidences_m_eps)
    ...
    def update_run_length_log_distribution(self, t):
        ...
        run_length_log_distr = utils.logsumexp(model_rl_log_distributions, axis=0)
    ...
    def update_all_joint_log_probabilities(self, y, t):
        ...
        if (
            ...
            if at_least_one_model_initialized:
                ...
                model_sums_derivatives, model_sums_derivatives_sign = utils.logsumexp(
                    a=all_log_alpha_derivatives,
                    b=all_log_alpha_derivatives_sign,
                    return_sign=True,
                    axis=0,
                )
                model_sums = utils.logsumexp(a=all_log_probs, axis=0)
            ...
            if initialization_required:
                ...
                *joint_probabilities*, (ii) the predictive distributions (i.e.,
                ...
                if warmed_up:
                    ...
                    log_CP_evidence = utils.logsumexp(
    ...
    def update_alpha_rld(self, y, t, update=True):
        ...
        sum_derivatives, sum_derivatives_sign = utils.logsumexp(
            a=all_log_alpha_derivatives,
            b=all_log_alpha_derivatives_sign,
            return_sign=True,
        )
        ...
        if False:  # self.loss_type = "posterior_expectation"
            ...
            for m, model in zip(range(0, self.Q), self.model_universe):
                ...
                post_prob_log = utils.logsumexp(a=np.array([post_prob_log, sum_]))
                """rlm log derivative"""
                sum_, sign_ = utils.logsumexp(
                    a=np.array(
                        [
                            run_length_and_model_log_der[
                                m, model_indices_indicators_relative_to_all_run_lengths
                ...
                post_prob_der_log, post_prob_der_log_sign = utils.logsumexp(
    ...
    def update_log_evidence(self):
        ...
        self.log_evidence = utils.logsumexp(
