...
def ensure_interface_not_busy(dev, interface):
    if dev.is_kernel_driver_active(interface.bInterfaceNumber):
        try:
            ...
            print(
                f"Could not detach kernel driver from interface({interface.bInterfaceNumber}): {e}",
                file=sys.stderr,
            )
            sys.exit(1)
...
# State for accumulating in decode
energy = 0.0
capacity = 0.0


def decode(data, calculate_crc, time_interval, alpha, buffer):
    global energy
    global capacity
    temp_ema = None
    ...
    for i in range(4):
        ...
        # )
        write_to_buffer(buffer, (t, voltage, current, energy))
...
def power_log(queue, stop_list):
    ...
    parser.add_argument(
        "--verbose",
        type=str2bool,
        help="Show some extra initialization information",
        default=False,
    )
    parser.add_argument(
        "--alpha", type=float, help="Set temperature smoothing factor", default=0.9
    )
    ...
    if args.crc:
        try:
            ...
            print(
                "Warning: crc package not found. crc checks will not be performed",
                file=sys.stderr,
            )
            crc_calculator = None
        except Exception as e:
            print(
                "Warning: crc calculator got exception: {e}, disabling crc checks",
                file=sys.stderr,
            )
            crc_calculator = None
    ...
    max_size = sps * 1000  # 1000 seconds of data
    buffer = queue
    ...
    if args.verbose:
        print(
            f"Ensuring all interfaces not busy and detaching kernel driver, if needed …",
            file=sys.stderr,
        )
    ...
    # print("time sample_in_packet V A dp_V dn_V temp_C energy_Ws cap_As")

    time.sleep(0.01)
    # refresh = 0.003
    refresh = 1.0 if is_fnb58_or_fnb48s else 0.003
    # 1 s for FNB58 / FNB48S, 3 ms for others
    ...
    while not stop:
        try:
            ...
            decode(data, crc_calculator, time_interval, alpha, buffer=buffer)
            ...
            print(
                "Terminating due to the keyboard interrupt, please wait until draining is complete …",
                file=sys.stderr,
            )
            stop = True
        ...
        if os.path.exists("fnirsi_stop"):
            stop = True
        stop = stop or stop_list[0]

    print("Closing USB logger")
    try:
        while True:
            ...
            if data:
                if args.verbose:
                    print(
                        f"During termination drained {len(data)} bytes", file=sys.stderr
                    )
